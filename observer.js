...
var fetch_statuses = function(user_id){
    // 删除已获取的用户消息
    redis_client.del("user_messages:"+user_id);
    console.log("Start fetching messages for user "+user_id);
    redis_client.hmset("user_info:"+user_id, "export_status", "fetching");
    ...
    redis_client.hgetall("user_info:"+user_id, function(err, user_info){
        ...
        {
            console.log("Failed to export user "+user_id+" messages, "+
                "access_token or access_secret null");
            redis_client.hmset("user_info:"+user_id, "export_status", "error");
            return;
        }
        var fetch_one_page = function(page){
            if(!page){
                page = 0;
            }
                ...
                function (error, data, response) {
                    if(error){
                        ...
                        console.log("Failed to get user info, error:"+
                            error+"data: "+data+"response: "+response);
                        redis_client.hmset("user_info:"+user_id,
                            "export_status", "interrupted");
                        return;
                    }
                    ...
                    for(var message_index in user_timeline){
                        redis_client.rpush("user_messages:"+user_id,
                                JSON.stringify(user_timeline[message_index]));
                    }
                    setTimeout(fetch_one_page, 2500, ++page);
                }
            );        
        };
        // 从第0页开始，逐页获取用户的消息
...
var check_queue = function(config){
    if(!config 
            || !config.redis 
            || !config.redis.client){
        return;
    ...
    redis_client.llen("export_waiting_queue", 
        function(err, obj){
            if(err || obj <= 0){
                return;
            }
            // 从等待队列中获取一个用户ID
            redis_client.rpop("export_waiting_queue",
                function(err, user_id){
                    if(err){
                        return;
                    }
                    fetch_statuses(user_id);
                    return;
                }
            );
        }
