...
exports.do_oauth = function(req, res, next){
    ...
    oa.getOAuthRequestToken(
            function(error, oauth_token, oauth_token_secret, results){
                ...
                var responseJSON = {};
                responseJSON["oauth_url"] =
                    "http://m.fanfou.com/oauth/authorize?oauth_token="+
                    oauth_token+"&oauth_callback="+config.server_domain+
                    ":"+config.server_port+"/oauth/callback";
                res.send(responseJSON);
                return;
            });
};
exports.oauth_callback = function(req, res, next){
    if(!req.query
            ...
            || !req.session.user
            || !req.session.user.oauth_token
            || !req.session.user.oauth_token_secret)
    {
        next();
        return;
    }
...
exports.is_oauthed = function(req, res){
    ...
    if(req.session.user 
            ...
            && req.session.user.oauth_access_token_secret)
    {
